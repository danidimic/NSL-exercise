...
//calcolo della deviazione standard
double devstd_media(int N, double sum, double sum2){
	double e = sum2/N - pow(sum,2);
	return sqrt( e/(N-1) );
}
...
//integrale Monte Carlo uniform sampling
double uniform_Monte_Carlo(double a, double b, int N, double (*func)(double), Random rnd){
	double x, sum = 0;
	for(int i=0; i<N; i++){
		x = rnd.Rannyu(a, b);
		sum += (*func)(x);
